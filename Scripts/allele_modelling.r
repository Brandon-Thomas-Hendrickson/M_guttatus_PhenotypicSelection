...
p6 <- ggplot(results_by_trait[["pollen_avg_std"]], aes(x = Generations, y = Final_Allele_Frequency, group = factor(Density), color = factor(Density))) +
    geom_line() + theme_light() + 
    labs(title = paste("Pollen Number"), x = NULL, y = NULL) +
    scale_color_manual(name = "Density", values = c("#0e0325", "#0ac9db", "blue", "purple", "red", "magenta")) +
    theme(legend.position = "none",, plot.title = element_text(hjust = 0.5)) +
    geom_vline(aes(xintercept = gen[gen$Trait == "pollen_avg_std" & gen$Density == 2, "Generations"][1]), linetype = "dashed", color = "#0e0325") +
    geom_vline(aes(xintercept = gen[gen$Trait == "pollen_avg_std" & gen$Density == 3, "Generations"][1]), linetype = "dashed", color = "#0ac9db") +
    geom_vline(aes(xintercept = gen[gen$Trait == "pollen_avg_std" & gen$Density == 4, "Generations"][1]), linetype = "dashed", color = "blue") +
    geom_vline(aes(xintercept = gen[gen$Trait == "pollen_avg_std" & gen$Density == 5, "Generations"][1]), linetype = "dashed", color = "purple") +
    geom_vline(aes(xintercept = gen[gen$Trait == "pollen_avg_std" & gen$Density == 6, "Generations"][1]), linetype = "dashed", color = "red") +
    geom_vline(aes(xintercept = gen[gen$Trait == "pollen_avg_std" & gen$Density == 7, "Generations"][1]), linetype = "dashed", color = "magenta")
...
p9 <- ggplot(results_by_trait[["veg_weight_std"]], aes(x = Generations, y = Final_Allele_Frequency, group = factor(Density), color = factor(Density))) +
    geom_line() + theme_light() + 
    labs(title = paste("Biomass"), x = "Generations", y = NULL) +
    scale_color_manual(values = c("#0e0325", "#0ac9db", "blue", "purple", "red", "magenta")) +
    theme(legend.position = c(0.8,0.5), plot.title = element_text(hjust = 0.5), legend.title = element_blank()) +
    geom_vline(aes(xintercept = gen[gen$Trait == "veg_weight_std" & gen$Density == 2, "Generations"][1]), linetype = "dashed", color = "#0e0325") +
    geom_vline(aes(xintercept = gen[gen$Trait == "veg_weight_std" & gen$Density == 3, "Generations"][1]), linetype = "dashed", color = "#0ac9db") +
    geom_vline(aes(xintercept = gen[gen$Trait == "veg_weight_std" & gen$Density == 4, "Generations"][1]), linetype = "dashed", color = "blue") +
    geom_vline(aes(xintercept = gen[gen$Trait == "veg_weight_std" & gen$Density == 5, "Generations"][1]), linetype = "dashed", color = "purple") +
    geom_vline(aes(xintercept = gen[gen$Trait == "veg_weight_std" & gen$Density == 6, "Generations"][1]), linetype = "dashed", color = "red") +
    geom_vline(aes(xintercept = gen[gen$Trait == "veg_weight_std" & gen$Density == 7, "Generations"][1]), linetype = "dashed", color = "magenta")# Arrange the plots into one graphic
...
p6 <- ggplot(results_by_trait_sub[["pollen_avg_std"]], aes(x = Generations, y = Final_Allele_Frequency, group = factor(Density), color = factor(Density))) +
    geom_line() + theme_light() + 
    labs(title = paste("Pollen Number"), x = NULL, y = NULL) +
    scale_color_manual(name = "Density", values = c("#0e0325", "#0ac9db", "blue", "purple", "red", "magenta")) +
    theme(legend.position = "none",, plot.title = element_text(hjust = 0.5)) +
...
p9 <- ggplot(results_by_trait_sub[["veg_weight_std"]], aes(x = Generations, y = Final_Allele_Frequency, group = factor(Density), color = factor(Density))) +
    geom_line() + theme_light() + 
    labs(title = paste("Biomass"), x = "Generations", y = NULL) +
    scale_color_manual(name = "Density", values = c("#0e0325", "#0ac9db", "blue", "purple", "red", "magenta")) +
    theme(legend.position = c(0.8,0.5), plot.title = element_text(hjust = 0.5), legend.title = element_blank()) +
    ...
    geom_vline(aes(xintercept = gen[gen$Trait == "veg_weight_std" & gen$Density == 5, "Generations"][1]), linetype = "dashed", color = "purple") +
    geom_vline(aes(xintercept = gen[gen$Trait == "veg_weight_std" & gen$Density == 6, "Generations"][1]), linetype = "dashed", color = "red") +
    geom_vline(aes(xintercept = gen[gen$Trait == "veg_weight_std" & gen$Density == 7, "Generations"][1]), linetype = "dashed", color = "magenta")
...
# Define the function
change_in_allele_frequency <- function(P, s1, s2, s3, w1, w2, w3, generations, yield1, yield2, yield3) {
    for (i in 1:generations) {
        s <- w1 * s1 * yield1 + w2 * s2 * yield2 + w3 * s3 * yield3 # Calculate the weighted sum of the s values
        P <- P + s * P * (1 - P)
    }
    return(P)
}
...
for (i in 1:nrow(data)) {
    ...
    if (data$Density[i] >= 3) {
        ...
        for (generations in seq(1, 1000, 10)) {
            # Calculate the final allele frequency
            P_final <- change_in_allele_frequency(P, s1, s2, s3, w1, w2, w3, generations,yield1, yield2, yield3)

            # Add the results to the dataframe
            results_yield <- rbind(results_yield, data.frame(Trait = data$Trait[i], Density = density, Estimate = s1, Estimate_F = s2, Estimate_LF = s3, Generations = generations, Final_Allele_Frequency = P_final, stringsAsFactors = FALSE))
        }
    } else if (data$Density[i] == 2) {
        # Get the s values from the Estimate, Estimate_F, and Estimate_LF columns
        s1 <- 0
...
p6 <- ggplot(results_by_trait_yield[["pollen_avg_std"]], aes(x = Generations, y = Final_Allele_Frequency, group = factor(Density), color = factor(Density))) +
    geom_line() + theme_light() + 
    labs(title = paste("Pollen Number"), x = NULL, y = NULL) +
    scale_color_manual(name = "Density", values = c("#0e0325", "#0ac9db", "blue", "purple", "red", "magenta")) +
    theme(legend.position = "none",, plot.title = element_text(hjust = 0.5)) +
...
p9 <- ggplot(results_by_trait_yield[["veg_weight_std"]], aes(x = Generations, y = Final_Allele_Frequency, group = factor(Density), color = factor(Density))) +
    geom_line() + theme_light() + 
    labs(title = paste("Biomass"), x = "Generations", y = NULL) +
    scale_color_manual(name = "Density", values = c("#0e0325", "#0ac9db", "blue", "purple", "red", "magenta")) +
    theme(legend.position = c(0.8,0.5), plot.title = element_text(hjust = 0.5), legend.title = element_blank()) +
...
trait_list <- c("flower_number_1_std", "D2Flowering_std", "ff_height_std", "total_node_number_std", "prop_early_growth_std", "final_height_std", "veg_weight_std", "leaf_area_std")
...
# Remove the pollen_avg_std trait
